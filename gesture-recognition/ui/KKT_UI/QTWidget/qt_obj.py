...
class InfoDialog(QtWidgets.QDialog):
    def __init__(self, txt_list:list):
        super().__init__()
        self.setWindowTitle('H5 info')
        self.setModal(True)
        self.resize(500, 300)
        ly = QtWidgets.QVBoxLayout()
        self.setLayout(ly)
        scroll_wg = QtWidgets.QScrollArea()
        scroll_wg.setWidgetResizable(True)
        ly.addWidget(scroll_wg)
        self.te = QtWidgets.QTextEdit()
        self.te.setReadOnly(True)
        self.te.setFont(QtGui.QFont('Yu Gothic UI', 10))
        self.te.setText('\n'.join(txt_list))
        scroll_wg.setWidget(self.te)

        self.pb_ok = QtWidgets.QPushButton('OK')
        self.pb_ok.setFont(QtGui.QFont('Arial', 12))
        self.pb_ok.clicked.connect(self.close)
        ly.addWidget(self.pb_ok)
...
class SubWidgetQFrame(QtWidgets.QFrame):
    border_css = "#frame {border: 1px solid lightgrey; border-radius: 3px;}"
    default_css = "#frame {border: 1px solid transparent; border-radius: 3px;}"
    def __init__(self, is_border=True):
        super().__init__()
        self.setObjectName("frame")
        self.enableBorder(is_border)
        self.grid = QtWidgets.QGridLayout(self)
        self.setLayout(self.grid)
        self.grid.setContentsMargins(5, 5, 5, 5)

    def enableBorder(self, is_border:bool):
        if is_border:
            self.setStyleSheet(self.border_css)
        else:
...
class CollapsibleSection(QtWidgets.QFrame):
    def __init__(self, header_text, init_collapsed=False, is_top=False):
        ...
        self._button.setFont(QtGui.QFont('Yu Gothic UI', 12))
...
class InputThreshold(SubWidgetQFrame):
    ThresholdChanged = QtCore.Signal(object)
    ...
    def setup(self):
        ...
        self._lb_lower.setFont(QtGui.QFont("Yu Gothic UI",10))
        self._lb_upper = QtWidgets.QLabel('Upper Threshold :')
        self._lb_upper.setFont(QtGui.QFont("Yu Gothic UI",10))
        self._sb_lower = QtWidgets.QDoubleSpinBox(singleStep=0.1, maximum=1, minimum=0)
        self._sb_lower.setFont(QtGui.QFont("Yu Gothic UI",10))
        self._sb_upper = QtWidgets.QDoubleSpinBox(singleStep=0.1, maximum=1, minimum=0)
        self._sb_upper.setFont(QtGui.QFont("Yu Gothic UI",10))
        self.initPlots()

        self._sb_upper.valueChanged.connect(self._changeSpinValue)
        self._sb_lower.valueChanged.connect(self._changeSpinValue)
        ly_upper = QtWidgets.QHBoxLayout()
        ly_upper.addWidget(self._lb_upper)
        ly_upper.addWidget(self._sb_upper)
        ly_lower = QtWidgets.QHBoxLayout()
        ly_lower.addWidget(self._lb_lower)
        ly_lower.addWidget(self._sb_lower)

        self.grid.addLayout(ly_upper,0,0,1,2)
        self.grid.addLayout(ly_lower,1,0,1,2)
    ...
    def _changeSpinValue(self):
        if self._sb_lower.value() <= self._sb_upper.value():
            ...
            self.ThresholdChanged.emit(self.current_threshold)
        else:
            self._sb_lower.setValue(self.current_threshold[0])
            self._sb_upper.setValue(self.current_threshold[1])
            QtWidgets.QMessageBox.warning(None, 'Warning', 'lower threshold must be less than upper threshold !')
...
class GesRecordModeWidget(SubWidgetQFrame):
    ...
    def setup(self):
        ...
        self.ComboBox.currentIndexChanged.connect(self._selectComboBox)
        self._selectComboBox()
    ...
    def _selectComboBox(self):
        ...
        if self._duration != None:
            ...
            self._duration.deleteLater()
            self._min_gap.deleteLater()
            self._duration = None
            self._min_gap = None

        if self._box_list[self.ComboBox.currentIndex()] == 'Pre-define':
            self._duration = HTextLineWidget('Duration :', '20', border=True)
            self._min_gap = HTextLineWidget('Minimum Gap :', '10', border=True)
            self.grid.addWidget(self._duration, 1, 0, 1, 3)
            self.grid.addWidget(self._min_gap, 2, 0, 1, 3)
    ...
    def getDuration(self):
        if self._duration == None:
            return 0
        return int(self._duration.getLineText())

    def getMinGap(self):
        if self._min_gap == None:
            return 0
        return int(self._min_gap.getLineText())
...
class NumberOfGestureWidget(SubWidgetQFrame):
    ...
    def _genGestureList(self):
        ...
        self._ges_list =[]
        if num == 0:
            ges = HComboBoxWidget('Gesture 1 :', ['Background'], border=True)
            self.grid.addWidget(ges,1,0,1,2)
            self._ges_list.append(ges)
            return

        for i in range(num):
            ges = HComboBoxWidget('Gesture {} :'.format(i+1), l, border=True)
...
class CheckBoxListWidget(SubWidgetQFrame):
    check_Signal = QtCore.Signal(object)
    ...
    def _checkBox(self):
        for k, v in self._items.items():
            if v['item'] is self.sender():
                ...
                self.status_changed = True
                break
        self.check_Signal.emit(self._items)
        # print(self._items)
...
class SetScriptWidget2(SubWidgetQFrame):
    ...
    def _setScript(self):
        self.initStatus()
        self.enableWidget(False)
        self.SetScript_Signal.emit(self._setting_comboBox.currentText())
...
class ModeSelectWidget(SubWidgetQFrame):
    changeIndex = QtCore.Signal(object)
    PressStart_signal = QtCore.Signal(object)
    def  __init__(self, label='label', box_list=[], border=True):
        self._label_text = label
        self._box_list = box_list
    ...
    def setup(self):
        # self.grid.setColumnStretch(2, 1)
        self._Label = QtWidgets.QLabel(self._label_text)
        self._Label.setFont(QtGui.QFont("Yu Gothic UI",10))
        self._ComboBox = QtWidgets.QComboBox()
        self._ComboBox.setFont(QtGui.QFont("Yu Gothic UI",10))
        self._ComboBox.addItems(self._box_list)
    ...
    def _selectComboBox(self):
        # print(self._ComboBox.currentIndex())
        self.changeIndex.emit(self._ComboBox.currentText())
    ...
    def _pressStart(self):
        self.PressStart_signal.emit(self._ComboBox.currentText())
...
class SpinBoxListWidget(SubWidgetQFrame):
    change_Signal = QtCore.Signal(object)
    def __init__(self, items:dict, label=None, cols=1, border=True):
        self._items = items
        self._label_text = label
        self._cols = cols
    ...
    def _changeSpinBox(self):
        self.change_Signal.emit(self._items)
        for k, v in self._items.items():
            v['Value'] = v['Spin'].value()
        pass
    ...
    def _changeComboBox(self):
        for k, v in self._items.items():
            if v['Box'] == self.sender():
                ...
                break
        print(k ,v['Index'])
        self.changeIndex.emit(self._items)
