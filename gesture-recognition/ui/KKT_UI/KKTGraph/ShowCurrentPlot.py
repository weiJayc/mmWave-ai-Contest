from KKT_UI.KKTGraph.Base.KKTGraph import KKTGraph, DataPlotWidget
from PySide2 import QtWidgets, QtCore, QtGui
import pyqtgraph as pg
import numpy as np
from threading import Lock


class CurrentDistance(DataPlotWidget, KKTGraph):
    lock = Lock()
    def __init__(self):
        super().__init__(title='Distance vs Time')
        self.peak = np.zeros(3600)
        self.x =(np.asarray([i for i in range(3600)])-3600)/20
        self.XRange =[-3600/20-1, 0]
        self.setupGUI()

        self.plot_item.setRange(xRange=[-3600/20-1, -1], yRange=[5, 70])
        self.setLimits(xMax = 0, xMin=-180, yMin=0)
        pass

    def setupGUI(self):
        self.current = pg.PlotCurveItem(pen={'color':'b', 'width': 3})
        self.plot_item.addItem(self.current)
        self.getAxis('left').setLabel('Distance (cm)')
        self.getAxis('bottom').setLabel('time (s)')
        self.setMenuEnabled(False)
        self.showGrid(x = False, y = True, alpha = 0.3)
        self.current.setData(x=self.x, y=self.peak)
        # self.addItem(self.current)
        # self.sigRangeChanged.connect(lambda :self.fixAxis())


        pass
    def initPeak(self):
        self.XRange[0] = self.getAxis('bottom').range[0]
        self.setData(0)

    def setData(self, data:float):
        self.peak = np.roll(self.peak, -1)
        self.peak[-1] = data
        self.current.setData(x=self.x ,y=self.peak)
        self.plot_item.setYRange(max = np.max(self.peak) + 20, min = 5)
        if np.abs(self.XRange[0] - self.getAxis('bottom').range[0]) > 3:
            self.plot_item.setXRange(max=self.XRange[1], min=self.getAxis('bottom').range[0])
            self.XRange[0] = self.getAxis('bottom').range[0]

    def setTimes(self, time_per_sec:int):
        buffer_len = 180*time_per_sec
        self.peak = np.zeros(buffer_len)
        self.x = np.linspace(-180, 0, buffer_len)
        self.XRange = [-180, 0]

    def fixAxis(self):
        self.plot_item.setXRange(max=self.XRange[1], min=self.XRange[0])



if __name__ == '__main__':
    def data_update():
        global g_map
        # time.sleep(0.05)
        l = np.random.randint(0,60, 1)
        g_map.setData(l)


    # g_map = None
    app = QtWidgets.QApplication([])
    win = QtWidgets.QMainWindow()
    win.resize(800,600)

    g_map = CurrentDistance()
    g_map.setProportion(np.linspace(0, 100, 60))


    win.setCentralWidget(g_map)
    win.setWindowTitle('Unit test')
    win.show()

    qtimer = QtCore.QTimer()
    qtimer.timeout.connect(data_update)
    qtimer.start(40)
    app.exec_()
    qtimer.stop()