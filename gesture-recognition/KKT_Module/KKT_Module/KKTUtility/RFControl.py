...
class RFController():
    ...
    def setChirpNumber(cls, chirp: int, duration: int):  # AIoT
        ...
        Gap = int(duration * 5 - chirp - 1) & 0xFFFF
        SX = 0x0001 + ((Gap - 6) << 8) & 0xFFFF
        TX = 0x0001 + ((Gap - 2) << 8) & 0xFFFF
        RX1 = 0x0001 + ((Gap - 5) << 8) & 0xFFFF
        RX2 = 0x0001 + ((Gap - 4) << 8) & 0xFFFF
        RX3 = 0x0001 + ((Gap - 3) << 8) & 0xFFFF
    ...
    def getDuration(cls) -> int:  # AIoT
        ...
        duration = val * 0.2 + chirp * (0.1024 + 0.1024)  # t_gap*0.2 + t_chirp*(up chirp time + down chirp time)
