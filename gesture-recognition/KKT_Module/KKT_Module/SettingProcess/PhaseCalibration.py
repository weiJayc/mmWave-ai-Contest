...
class PhaseCalibration:
    ...
    def calibrate(self):
        ...
        self._phase_offset_RX1, self._phase_offset_RX2, temperature = kgl.ksoclib.getRXPhaseOffset()
...
class RXControl():
    ...
    def rewriteMuxConfig(cls, open_RX:int, **kwargs)->None:
        ...
        AI_mux = kwargs.get('AI_mux')
        tracking_mux = kwargs.get('tracking_mux')

        # set default mux
        if open_RX == 0b101:
            if AI_mux is None:
                AI_mux = 6
            if tracking_mux is None:
                tracking_mux = 6
        elif open_RX == 0b110:
            if AI_mux is None:
                AI_mux = 1
