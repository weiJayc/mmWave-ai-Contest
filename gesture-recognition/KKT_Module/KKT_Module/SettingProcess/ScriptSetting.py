...
class ScriptSetter:
    ...
    def configByDevice(self, script_dir, procList:list, weightDir=None, write_AI=True, write_RF=True):
        aiweightpath = None
        ...
        for plist in procList:
            ...
            if plist[0] == pls.RegSymbol:
                ...
                # aiweightpath = script_dir.get('Script_path') + r'\ai_acc_weight\sram_coe'
                print(aiweightpath)
                ...
                for f in plist[1]:
                    if weightDir == None:
                        aiwf =  os.path.normpath(os.path.join(script_dir.get('Script_path'), '.' + aiweightpath, f))
                        # aiwf = os.path.normpath(os.path.join(aiweightpath, f))
                    else:
                        aiwf =  os.path.normpath(os.path.join(weightDir, f))
                    print(aiwf)
                    if 'sram_coe' in aiweightpath:
                        kgl.ksoclib.setScriptAIWeight(aiwf, True)
                    elif 'sram_bin' in aiweightpath:
                        kgl.ksoclib.setAIWeightBinFile(aiwf)
            elif plist[0] == 'test':
                time.sleep(0.01)
